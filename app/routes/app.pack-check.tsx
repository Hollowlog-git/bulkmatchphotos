...
interface LineItem {
  ...
  variantTitle?: string; imageUrl?: string | null; productUrl?: string | null;
  productId?: string | null; unitPrice: number; originalUnitPrice: number; currencyCode: string;
}
...
interface Order {
  ...
  shippingWasFree: boolean; hasShippingDiscount: boolean; currencyCode: string;
  discountCodes: string[]; shippingAddress: ShippingAddress | null;
  fulfillmentOrders: FulfillmentOrder[]; lineItems: LineItem[];
}
interface PackItem extends LineItem {
  scanned: number; sortKey: string; orderNames: string[];
}

const PALETTE = [
  { bg: "#d4f57a", text: "#2d4a00" }, { bg: "#ffb3b3", text: "#5c0000" },
...
function channelBadge(channel: string) {
  if (channel === "Trade Me") return { label: "Trade Me", bg: "#00547a", text: "#fff" };
  if (channel === "Online Store") return { label: "Online Store", bg: "#008060", text: "#fff" };
  if (channel === "Shop App") return { label: "Shop App", bg: "#5c6ac4", text: "#fff" };
  return { label: channel, bg: "#888", text: "#fff" };
}
...
export default function PackCheck() {
  ...
  function saveSession(customer: string, items: PackItem[]) {
    try {
      ...
      for (const item of items) if (item.scanned > 0) scannedMap[item.sku || item.id] = item.scanned;
  ...
  const fetchOrders = useCallback(async (restoreSession = true) => {
    ...
    try {
      ...
      if (fetched.length > 0) {
        ...
        if (targetCustomer) selectOrderByCustomer(targetCustomer, fetched, session?.scannedMap);
      }
  ...
  function moveToNextOrder() {
    ...
    const nextIdx = customers.indexOf(currentCustomer) + 1;
  ...
  function selectOrderByCustomer(customer: string, orderList: Order[], scannedMap?: Record<string, number>) {
    ...
    for (const order of customerOrders) {
      for (const item of order.lineItems) {
        ...
        if (map.has(key)) {
          ...
          if (!map.get(key)!.orderNames.includes(order.name)) map.get(key)!.orderNames.push(order.name);
        } else {
    ...
    setTimeout(() => { (document.activeElement as HTMLElement)?.blur(); scanRef.current?.focus(); }, 200);
  }

  function selectOrder(value: string) {
    if (!value) { setSelectValue(""); setSelectedOrderIds([]); setPackItems([]); packItemsRef.current = []; clearSession(); return; }
    clearSession();
  ...
  async function fulfillOrders() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) {
        setFulfillMsg({ text: `❌ ${data.error}`, ok: false });
      } else {
        setFulfillMsg({ text: "✅ Fulfilled! Customer notified.", ok: true });
        clearSession();
        setTimeout(() => fetchOrders(false), 2000);
      }
    } catch (e: any) {
      setFulfillMsg({ text: `❌ ${e.message}`, ok: false });
    } finally {
      setFulfilling(false);
    }
  }
  ...
  const mostValuable = packItems.length > 0 ? [...packItems].sort((a, b) => (b.unitPrice * b.quantity) - (a.unitPrice * a.quantity))[0] : null;

  const sortedPackItems = [...packItems].sort((a, b) => {
    const aDone = a.scanned >= a.quantity, bDone = b.scanned >= b.quantity;
  ...
  function confirmScanDirect(raw: string) {
    ...
    const idx = items.findIndex(i =>
      (i.sku && (i.sku === sku || i.sku === raw.toUpperCase())) || i.title.toLowerCase() === raw.toLowerCase()
    );
    ...
    setPackItems(prev => {
      ...
      if (item.scanned > item.quantity) setScanMsg({ text: `⚠️ ${label} — OVER-SCANNED`, tone: "critical" });
      else if (item.scanned === item.quantity) setScanMsg({ text: `✓ ${label} complete`, tone: "success" });
      else setScanMsg({ text: `✓ ${label} (${item.scanned}/${item.quantity})`, tone: "success" });
  ...
  function markPackedDirect(item: PackItem) {
    ...
    setPackItems(prev => {
      ...
      updated[idx] = { ...updated[idx], scanned: updated[idx].scanned + 1 };
      const u = updated[idx];
      if (u.scanned > u.quantity) setScanMsg({ text: `⚠️ ${key} — OVER-SCANNED`, tone: "critical" });
      else if (u.scanned === u.quantity) setScanMsg({ text: `✓ ${key} complete`, tone: "success" });
      else setScanMsg({ text: `✓ ${key} (${u.scanned}/${u.quantity})`, tone: "success" });
      packItemsRef.current = updated;
      saveSession(selectValueRef.current, updated);
      return updated;
    });
  ...
  function undoLastDirect() {
    ...
    if (!h.length) { setScanMsg({ text: "Nothing to undo.", tone: "warning" }); return; }
  ...
  function resetScans() {
    ...
    setPackItems(reset); packItemsRef.current = reset;
    setHistory([]); historyRef.current = [];
  ...
  const groupedOptions = () => {
    ...
    for (const [customer, customerOrders] of sorted) {
      ...
      const ageDays = Math.floor((Date.now() - Math.min(...customerOrders.map(o => new Date(o.createdAt).getTime()))) / 86400000);
        ...
        <Layout.Section>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ flex: "1 1 300px", minWidth: 0 }}>
              {loadingOrders ? <InlineStack align="start"><Spinner size="small" /></InlineStack> : (
                <Select label="" labelHidden options={groupedOptions()} value={selectValue} onChange={selectOrder} />
              )}
            </div>
            {packItems.length > 0 && (
            ...
            <InlineStack gap="200" blockAlign="center">
              {totalUnfulfilledCount > 0 && (
                <div style={{ background: "#d72c0d", color: "#fff", fontWeight: 700, fontSize: 12, padding: "3px 9px", borderRadius: 12 }}>
            ...
            <div style={{ background: "#008060", borderRadius: 10, padding: "20px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 16 }}>
                ...
                <InlineStack gap="200">
                  ...
                  <Button size="large" onClick={moveToNextOrder}>Next Order →</Button>
                </InlineStack>
              </div>
              {fulfillMsg && (
                <div style={{ background: fulfillMsg.ok ? "rgba(255,255,255,0.15)" : "#d72c0d", borderRadius: 8, padding: "10px 16px", color: "#fff", fontWeight: 700, marginBottom: 12 }}>
                  {fulfillMsg.text}
                </div>
              )}
              {mostValuable && (
                <div style={{ background: "rgba(255,255,255,0.12)", borderRadius: 8, padding: "12px 16px", display: "flex", alignItems: "center", gap: 14 }}>
                  {mostValuable.imageUrl && <img src={mostValuable.imageUrl} alt={mostValuable.title} style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 6, flexShrink: 0 }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: "#ffd700", fontWeight: 700, fontSize: 12, textTransform: "uppercase", marginBottom: 2 }}>⭐ Most Valuable — Pack On Top</div>
              ...
              <div style={{ display: "flex", gap: 24, alignItems: "center", padding: "8px 0" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 15, color: "#333", marginBottom: 4 }}>
                    {nextItem.title}{nextItem.variantTitle && nextItem.variantTitle !== "Default Title" ? ` · ${nextItem.variantTitle}` : ""}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                    {/jap/i.test(nextItem.title) && <span style={{ background: "#dc2626", color: "#fff", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4, textTransform: "uppercase" }}>🇯🇵 Japanese</span>}
                    {nextItem.unitPrice > 0 && <span style={{ background: "#f0f0f0", color: "#333", fontSize: 13, fontWeight: 700, padding: "2px 10px", borderRadius: 4 }}>{fmt(nextItem.unitPrice, nextItem.currencyCode)}</span>}
                    {scanMsg.text && (
                  ...
                  <div style={{ fontSize: 13, color: "#888" }}>
                    {nextItem.scanned}/{nextItem.quantity} packed{nextItem.orderNames.length > 1 && ` · ${nextItem.orderNames.join(", ")}`}
                  </div>
                </div>
                <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                  <Button variant="primary" size="large" onClick={() => markPacked(nextItem)}>✓ Mark as packed</Button>
                    ...
                    <BlockStack gap="200">
                      <InlineStack gap="200" wrap blockAlign="center">
                        ...
                        {customerTotalOrders !== null && (
                          <span style={{ fontSize: 12, color: customerTotalOrders >= 5 ? "#008060" : "#888", fontWeight: customerTotalOrders >= 5 ? 700 : 400 }}>
                            {customerTotalOrders >= 5 ? "⭐ " : ""}{customerTotalOrders} order{customerTotalOrders !== 1 ? "s" : ""} total
                          </span>
                        )}
                      </InlineStack>
                      ...
                      <InlineStack gap="200" wrap>
                        {selectedOrders.map(o => {
                          const cb = channelBadge(o.channel);
                          const ageDays = Math.floor((Date.now() - new Date(o.createdAt).getTime()) / 86400000);
                          return (
                            <InlineStack key={o.id} gap="100" blockAlign="center">
                              <span style={{ background: cb.bg, color: cb.text, fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 4 }}>{cb.label}</span>
                              <span style={{ fontSize: 11, color: ageDays >= 2 ? "#d72c0d" : "#888" }}>{o.name} {timeAgo(o.createdAt)}</span>
                            </InlineStack>
                          );
                        })}
                      </InlineStack>
              ...
              {packItems.length > 0 && (
                <Card>
                  <BlockStack gap="300">
                    ...
                    <TextField label="" labelHidden value={scanValue} onChange={handleScanChange}
                    ...
                    <BlockStack gap="200">
                      {filteredItems.length === 0 ? (
                        ...
                        filteredItems.map(item => {
                          ...
                          return (
                            <div key={item.id}
                              style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", borderRadius: "8px", border: `1px solid ${over ? "#d72c0d" : isNext ? c.bg : isMostVal ? "#f0c040" : done ? "#e3e3e3" : partial ? "#e1a900" : "#e3e3e3"}`, borderLeft: `5px solid ${c.bg}`, background: over ? "#fff4f4" : isNext ? `${c.bg}22` : isMostVal ? "#fffbeb" : done ? "#f8f8f8" : "#ffffff", opacity: done ? 0.55 : 1, transition: "all 0.15s" }}>

                              {/* Image — clickable to product */}
                              {item.productUrl ? (
                                <a href={item.productUrl} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ flexShrink: 0 }}>
                                  {item.imageUrl ? (
                                    <img src={item.imageUrl} alt={item.title} style={{ width: 40, height: 40, objectFit: "cover", borderRadius: 5, opacity: done ? 0.5 : 1, display: "block" }} />
                                  ) : (
                                    <div style={{ width: 40, height: 40, borderRadius: 5, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: c.text }}>
                                ...
                                <div style={{ flexShrink: 0 }}>
                                  {item.imageUrl ? (
                                    <img src={item.imageUrl} alt={item.title} style={{ width: 40, height: 40, objectFit: "cover", borderRadius: 5, opacity: done ? 0.5 : 1 }} />
                                  ) : (
                                    <div style={{ width: 40, height: 40, borderRadius: 5, background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: c.text }}>
                                      {item.sku ? item.sku.substring(0, 2) : "?"}
                                    </div>
                              ...
                              <div style={{ minWidth: 0, flex: 1, cursor: item.sku ? "pointer" : "default" }}
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2, flexWrap: "wrap" }}>
                                  ...
                                  {item.productUrl && (
                                    <a href={item.productUrl} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 10, color: "#2563eb", textDecoration: "none" }}>↗</a>
                                  )}
                                </div>
